...
class LessonsBloc extends SafeBloc<LessonsEvent, LessonsState> {
  ...
  LessonsBloc({required this.coursesRepository}) : super(const LessonsState()) {
    on<LessonsLoaded>(_onLessonsLoaded);
    on<CurrentLessonChanged>(_onCurrentLessonChanged);
    on<ErrorOcurred>(_onErrorOccurred);
  }

  void _onLessonsLoaded(LessonsLoaded event, Emitter<LessonsState> emit) {
    emit(state.copyWith(
  ...
  void _onErrorOccurred(ErrorOcurred event, Emitter<LessonsState> emit) {
    emit(state.copyWith(
  ...
  void changeToNextLesson() {
    ...
    final lesson =
        state.lessons.where((l) => l.order == (state.currentLesson.order + 1));
    if (lesson.isNotEmpty) {
      final newLesson = lesson.toList()[0];
      add(CurrentLessonChanged(
        newLesson,
