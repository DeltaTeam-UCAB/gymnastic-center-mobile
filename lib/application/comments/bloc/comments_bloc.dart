...
class CommentsBloc extends Bloc<CommentsEvent, CommentsState> {
  ...
  void _onCommentsCompleted(
      CommentsCompleted event, Emitter<CommentsState> emit) {
    emit(state.copyWith(status: CommentsStatus.completed));
  }
  ...
  Future<void> deleteLike(String commentId, bool userliked) async {
    final commentDislikedResult =
        await commentsRepository.deleteLikeByCommentId(commentId);
  ...
  Future<void> likeComment(String commentId, bool userliked) async {
    final commentLikedResult =
        await commentsRepository.likeCommentById(commentId);
  ...
  Future<void> loadNextPageByPostId(String postId) async {
    ...
    add(LoadingStarted());
    final commentsResult = await commentsRepository.getCommentsByPostId(postId,
