...
class SuscriptionRepositoryImpl extends SuscriptionRepository{
  ...
  SuscriptionRepositoryImpl(
    this.suscriptionsDatasource
  );
  
  @override
  Future<Result<CourseProgress>> getProgressByCourseId(String courseId)  async{
    try {
      final courseProgress = await suscriptionsDatasource.getProgressByCourseId(courseId);
  ...
  @override
  Future<Result<bool>> markCompletedProgressLesson(String courseId, String lessonId) async {
    try {
      await suscriptionsDatasource.markCompletedProgressLesson(courseId, lessonId);
      return Result.success(true);
    } catch (e) {
      return Result.fail(e as Exception);
    }
  ...
  @override
  Future<Result<bool>> markEndProgressLesson(String courseId, String lessonId, int totalSeconds, int secondsViewed) async {
    try {
      await suscriptionsDatasource.markEndProgressLesson(courseId, lessonId, totalSeconds, secondsViewed);
      return Result.success(true);
    } catch (e) {
      return Result.fail(e as Exception);
    }
  ...
  @override
  Future<Result<List<CourseProgress>>> getSuscribedCourses(int page, {int? perPage = 5}) async {
    try {
      final coursesSuscribed = await suscriptionsDatasource.getSuscribedCourses(page, perPage: perPage);
